...
* 用模拟结果绘制散点图和拟合曲线图
```
graph twoway (lowess diff sample_id,legend(ring(1) pos(6)label(1 "fitting line"))) (scatter diff sample_id,ms(circle) msize(vsmall) title("law of large numbers") xtitle("sampling probability") ytitle("different between sample and population")legend(ring(1) pos(6)label(2 "scatter")))
```
