...
- 假设检验
- SPSS
title: 第九节 抽样分布、区间估计与均值检验
---

#### 从理论到实践
> **辅助知识点：**
> 在上一讲中，我们通过模拟，直观地理解了总体、样本和抽样分布这三大核心概念。我们知道了抽样分布是连接我们手中样本与未知总体的理论桥梁。
> 
> 本讲我们将学习如何：
...
*   **场景设定**
    *   首先，我们将完整的CLDS数据集**视为“总体”**。
    *   然后，我们从中随机抽取一个**样本 (n=500)**。
    *   最后，我们将计算理论上的**抽样分布**参数。
...
* 计算总体均值(μ).
AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /pop_mean = MEAN(income).
* 计算总体方差(σ²)，其定义是离差平方的均值.
COMPUTE diff_sq = (income - pop_mean)**2.
AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /pop_var = MEAN(diff_sq).
* 计算总体标准差(σ).
COMPUTE pop_sd = SQRT(pop_var).
EXECUTE.

* 显示结果.
SUMMARIZE /TABLES=pop_mean pop_var pop_sd /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
```
![总体参数](https://stat4soc.netlify.app/images/9.1.png)

*   **第二步：抽取样本，并计算样本统计量 (Sample Statistics)**
    *   现在我们从1462个“总体”个案中，抽取一个n=500的样本。
```spss
* 重新加载完整数据以进行抽样.
GET FILE "clds2016_i.sav".
SELECT IF (I3a_6 >= 0).
...
* 注意：这里我们先手动计算分母为n的“有偏”方差.
AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /sam_mean = MEAN(income).
COMPUTE sam_diff_sq = (income - sam_mean)**2.
AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /sam_var_n = MEAN(sam_diff_sq).
COMPUTE sam_sd_n = SQRT(sam_var_n).
EXECUTE.
SUMMARIZE /TABLES=sam_mean sam_var_n sam_sd_n /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
```
...
*   **第三步：计算抽样分布的参数 (Sampling Distribution Parameters)**
    *   **情况一：当总体已知时 (理论情况)**
    ...
    EXECUTE.
    SUMMARIZE /TABLES=sampling_mean_pop sampling_var_pop sampling_sd_pop /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
    ```
    ...
    *   **情况二：当总体未知时 (现实情况)**
        ...
        *   **方法A：手动计算 (应用贝塞尔校正)**
            ...
            > 当我们使用样本方差来估计总体方差时，直接计算的样本方差（分母为n）会倾向于低估真实的总体方差。为了得到一个更准确的**无偏估计**，我们需要将分母从`n`调整为`n-1`。这个小小的调整，就是贝塞尔校正。
        ```spss
        /* 假设我们只有那个n=500的样本... */
        COMPUTE sampling_mean_sam1 = sam_mean.
        * 计算离差平方和.
        AGGREGATE /OUTFILE=* MODE=ADDVARIABLES /sam_diff_sum = SUM(sam_diff_sq).
        ...
        * 估计标准误.
        ...
        EXECUTE.
        SUMMARIZE /TABLES=sampling_mean_sam1 sampling_var_sam1 sampling_sd_sam1 /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
        ```
        ![抽样分布参数-手动估计](https://stat4soc.netlify.app/images/9.4.png)

        *   **方法B：利用SPSS内置命令与OMS (推荐)**
        ...
        * 假设我们仍在n=500的样本数据集中.
        ...
        DESCRIPTIVES income /STATISTICS=MEAN STDDEV VARIANCE SEMEAN.
        OMSEND.

        * 读取捕获的结果.
        GET FILE 'sample_descriptives.sav'.
        RENAME VARIABLES (Mean_Statistic=sam_mean) (Std. Deviation_Statistic=s) (Variance_Statistic=s_squared) (Std. Error of Mean_Statistic=SE_mean).
        ...
        * 整理并计算.
        ...
        EXECUTE.
        SUMMARIZE /TABLES=sampling_mean_sam2 sampling_var_sam2 sampling_sd_sam2 /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
        ```
