...
#### 案例（1-1）：探索2015年主职总收入的影响因素
* 第一步：对2015年主职总收入、年龄与教育年限变量进行处理
```
select if (I3a_6  > 0).
...
#### 3.理解控制变量的本质
#### 案例（3-1）：探索2015年主职总收入与教育年限的关系
* 第一步：构造关于在教育年限（核心自变量）与主职总收入的回归模型，即基础模型或基准模型
* 第二步：在基础模型中加入年龄项（控制变量），构造拓展模型。拓展模型的作用是剔除年龄的作用，以获得教育年限对主职总收入的净影响
* 注意：
	* 核心自变量和控制变量都是自变量
...
#### 4.处理自变量与因变量的非线性关系
#### 案例（4-1）：探索2015年主职总收入与年龄的关系
* 查看不同年龄组的收入差异
```
descriptives age.
recode age (16 thru 30 = 1)(31 thru 40 = 2)(41 thru 50 = 3)(51 thru 60 = 4)(61 thru max = 5)(else = sysmis) into age_group.
...
#### 案例（4-2）：通过散点图探索2015年主职总收入与年龄的关系
* 利用图表构建器绘制散点图和拟合线
```
ggraph 
...
begin gpl
	...
	GUIDE: text.title(label("2015年劳动者年龄与主职总收入关系")) 
...
#### 案例（4-3）：通过回归分析探索2015年主职总收入与年龄的关系
* 第一步：构建年龄平方项与收入的线性模型
```
regression 
	/statistics coeff
...
#### 案例（6-1）：利用多元回归模型探索2015年主职总收入的影响因素
```
regression 
	/statistics coeff ci(95) 
	/dependent log_income 
