...
*   **点选操作**：分析 → 回归 → 线性 → ... → 点击“统计” → 在“残差”区域勾选“德宾-沃森” → 继续 → 确定。
*   **语法操作**：
```spss
REGRESSION 
	/DEPENDENT log_income 
...
*   通过**曲线估算**功能，可以直观地比较线性和二次曲线的拟合效果。
*   **点选操作**：分析 → 回归 → 曲线估算 → 放入因变量和自变量 → 在模型中勾选“线性”和“二次” → 确定。
*   **语法操作**：
```spss
CURVEFIT
...
SUMMARIZE /TABLES=turn_point /FORMAT=VALIDLIST NOCASENUM /CELLS=FIRST.
```
