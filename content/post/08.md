...
*   使用`GGRAPH`和`GPL`的过程有三个步骤
1.  **准备(`GGRAPH`)**：首先需要决定这幅画要用到哪些数据（变量），以及是否需要对这些数据做一些初步的“调色”（如计算均值、计数等）。
2.  **规划 (`BEGIN GPL...END GPL.` 的 `SOURCE` 和 `DATA` 部分)**：需要明确定义画中的基本元素是什么，例如，“横轴代表分类变量‘地区’”、“纵轴代表连续变量‘收入’”。
3.  **绘制 (`BEGIN GPL...END GPL.` 的 `GUIDE`, `SCALE`, `ELEMENT` 部分)**：开始绘制具体的图形元素，比如“画一个条形图”、“画一条拟合线”、“给坐标轴加上标签”、“给图取个标题”等等。
...
**第二步：`BEGIN GPL ... END GPL.` 代码块 —— 图形绘制**
