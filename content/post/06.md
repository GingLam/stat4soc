...
*   **基尼系数的计算**
    *   基尼系数在几何上被定义为：**绝对平等线与劳伦兹曲线所围成的面积（A-B）**，占**绝对平等线与坐标轴所围成的总面积A** 的比例。
    *   由于总面积A恒为0.5，所以基尼系数也可以表示为 `Gini = A-B / 0.5`。
...
CREATE cum_income_share = CSUM(income_share).

* 步骤六：应用基尼系数公式的求和部分，对所有人的计算部分求和，并得出最终基尼系数.
